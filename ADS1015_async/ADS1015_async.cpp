...
byte ADS1015_async::poll()
{
  ...
  byte i;
  byte j;
  int result;     // raw conversion value
  int absResult;  // raw absolute value
  ...
  if (result & 0x0800) {
    ...
    result = -(result ^ 0xFFF);       // toggle bits 0:11 for the absolute value & make minus
  }
  ...
  // caluculate the actual _voltage and store for the read() function
  raw_data = result;

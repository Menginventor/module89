    ...
    if (param_ch_count == 0) {
      ...
      if ((ch >= '0' && ch <= '9') || ch == '-' || ch == '+') {
        ...
        if (numeric_point_flag) {
          ...
          cmd_param[param_count] += ch;
          numeric_point_flag = true;
        }

      }
      else if (ch >= 'A' && ch <= 'Z') {
        Serial.println(F("New param ,2"));
        param_count++;
        param_ch_count = 0;
        numeric_point_flag = false;
        /**/
        cmd_param[param_count] += ch;
      }
      param_ch_count++;
